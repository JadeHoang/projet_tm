...
```
Cette partie prendre beaucoup de temps à extraire les vecteurs de mots.
Donc soit vous patientez lors de l'exécution, soir vous chargez le fichier embeddings.RData
Par défaut, j'ai chargé via .RData pour knit vers HTML.
...
#matrice similarité
#exemple chapitre 20
chap_20 <- data_chapter[which(data_chapter$chapitre == 20),]
chap_20$line <- seq(nrow(chap_20))
sim_mat <- matrix(0,nrow = nrow(chap_20),
                  ncol=nrow(chap_20))
...
for (i in seq(nrow(chap_20))){
  for (j in seq(nrow(chap_20))){
    if (i != j){
...
##Words pairs
```{r}
section_words <- data_chapter %>%
  select(-c(line, chapitre, partie)) %>%
